...
Для любого элементарного языка $n$-кортежей $\formallang{}$ существует константа $K > 0$ такая, что:
\begin{itemize}
\item для каждого кортежа термов $\tuple{g_1,\ldots,g_n} \in \formallang{}$,
\item для любого $i$ такого, что $\height{g_i} > K$,
\item для всех бесконечных сортов $\sigma \in \sorts$ и
\item для всех путей $p$ в терме $g$ с глубиной большей $K$
\item существует набор конечных последовательностей путей $P_j$,
\item где $p \in P_i$ и для всех $p_1, p_2 \in \bigcup_j P_j$ верно, что $p_1(g) = p_2(g)$,
\item и найдётся константа $N \geq 0$ такая, что
\item для любого терма $t$ сорта $\sigma$ c $\height{t} > N$ имеет место следующее:
\end{itemize}
...
Пусть есть некоторая расширяющая АТД-сигнатура и $\formallang{}$~--- элементарный язык $n$-кортежей с ограничениями на размер теремов. Тогда справедливо, что существует константа $K > 0$ такая, что:
\begin{itemize}
\item для каждого кортежа термов $\tuple{g_1,\ldots,g_n} \in \formallang{}$,
\item для любого $i$ такого, что $\height{g_i} > K$,
\item для всех бесконечных сортов $\sigma \in \sorts$ и
\item для всех путей $p$ в терме $g$ с глубиной большей $K$
\item существует бесконечное линейное множество $T \subseteq \sizesOfSort{\sigma}$ такое, что
\item для любых термов $t$ сорта $\sigma$ с размером $\sizename{}(t) \in T$,
\item существует набор последовательностей путей $P_j$, в котором ни один путь не является суффиксом пути $p$,
\item и набор последовательностей термов $U_j$ такие, что
\end{itemize}
