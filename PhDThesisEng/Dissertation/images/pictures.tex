...
Let $\formallang{}$ be an elementary language of $n$-tuples. Then, there exists a constant $K > 0$ satisfying: for every $n$-tuples of ground terms $\tuple{g_1,\ldots,g_n} \in \formallang{}$,
for any $i$ such that $\height{g_i} > K$,
for all infinite sorts $\sigma \in \sorts$ and for all paths $p$ with a length greater than $K$,
there exist finite sets of paths $P_j$ such that $p \in P_i$,
for all $p_1, p_2 \in \bigcup_j P_j$ it is true that $p_1(g) = p_2(g)$,
and there is $N \geq 0$, such that for all $t$ of sort $\sigma$ with $\height{t} > N$ it holds that:
...
for every $n$-tuple of ground terms $\tuple{g_1,\ldots,g_n} \in \formallang{}$,
for any $i$, such that $\height{g_i} > K$,
for all infinite sorts $\sigma \in \sorts$,
and for all paths $p \in \leafpos{\sigma}{g_i}$ with length greater than $K$,
there exists an infinite linear set $T \subseteq \sizesOfSort{\sigma}$, such that
for all terms $t$ of sort $\sigma$ with sizes $\sizename{}(t) \in T$,
