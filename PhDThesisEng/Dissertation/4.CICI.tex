...
The method presented in Section~\cref{sec:cici/inference} is an extension of a counterexample-guided abstraction refinement (\cegar{}) algorithm for inference of combined invariants. The modification involves a collaborative information exchange with the invariant inference algorithm for the class with which it is combined (the core idea is described in Section~\cref{sec:cici/idea}).
...
However, it has combined invariants in $\elemclass{}\uplus\regclass{}$. That is, for any programs $p, t : Prog$ function $\texttt{ok}(p)$ returns \texttt{true} if we have
